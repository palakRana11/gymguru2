...
const ai = new GoogleGenAI({ apiKey: "AIzaSyDKtzVB5adiPnGCVyhfYqwaV0KOedJMc2Y" });
...
export default function WorkoutPlan() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const prompt = `Create a 1 week workout plan for a person to achieve the goal "${goal}" in ${minutes} minutes per day. Make each week's plan different and structure it as a table. Format workout names in bold. Keep the tone friendly.`;

      // Changed to ai.models.generateContent to match working component
      const response = await ai.models.generateContent({
        model: "gemini-2.0-flash", // Use same model as working component
        contents: [{
          role: "user",
          parts: [{ text: prompt }]
        }]
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-6">
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-xl bg-gray-800 p-6 rounded-xl shadow-lg mb-6">
        ...
        <input
          ...
          required
        />
        <button type="submit" className="w-full bg-green-400 text-black p-3 rounded hover:bg-green-500">
          Generate Plan
        </button>
      </form>
      ...
      <div className="w-full max-w-5xl bg-gray-800 p-6 rounded-xl shadow-xl">
        {loading ? (
          ...
          plan ? renderWorkoutTable(plan) : null
        )}
      </div>
    </div>
  );
}
//AIzaSyDKtzVB5adiPnGCVyhfYqwaV0KOedJMc2Y
