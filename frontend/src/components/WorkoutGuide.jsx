  ...
  useEffect(() => {
    const fetchGuide = async () => {
      try {
        ...
        let rawText = response?.data?.reply || "No guide found.";
        ...
        rawText = rawText.replace(
          /(<li[\s\S]*?<\/li>)/g,
          "<ul class='list-disc list-inside pl-4 my-2'>$1</ul>"
        );
        ...
        rawText = rawText
          .split(/\n{2,}/)
          .map((block) => {
            if (block.startsWith("<h2") || block.startsWith("<ul"))
              return block;
            return `<p class='mb-4 leading-relaxed'>${block
              .trim()
              .replace(/\n/g, "<br />")}</p>`;
          })
          .join("");
        ...
        setGuide(
          "<p>Something went wrong while generating your guide. Try again later.</p>"
        );
      }
      setLoading(false);
    };
